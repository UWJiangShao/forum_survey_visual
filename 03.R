star_adult_hwdc <- merged_df %>%
  filter(program == "STAR_Kids", measure == "Getting Care Quickly", !is.na(MCO), MCO != 'Cigna-HealthSpring')
...
state_level <- merged_df %>%
  filter(measure == "Getting Care Quickly", program == "STAR_Kids") %>%
...
ggplot(combined_data, aes(x = as.numeric(year), y = avg_score, color = MCO)) +
  ...
  geom_line(data = national_level_child_GCQ, aes(x = year, y = score), color = "purple", linetype = "dashed", size = 1.7) +
  
  labs(title = "Getting Care Quickly (GCQ)\nState Level v.s AHRQ National Level in STAR Kids",
       x = NULL,
       y = "Score",
  ...
  geom_text(aes(x = 2023, y = 0.66, label = "AHRQ\nLevel"), 
